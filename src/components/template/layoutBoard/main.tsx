...
function MainComponent({ children }: { children: React.ReactNode }) {
    ...
    return (
        <main className=' relative w-full'>
            <header className={` ${open ? "w-[calc(100%-18rem)]" : "w-[calc(100%-4rem)]"} ${isMobile && "w-full"} z-50 backdrop-blur-sm px-10 py-5 fixed flex justify-between items-center gap-10`}>
                <TrigerSidebar />
                <div>
                    <Popover>
                        <PopoverTrigger>
